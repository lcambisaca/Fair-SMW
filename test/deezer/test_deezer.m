...
for run = 1:numRuns
    ...
    for i = 1:length(krange)
        ...
        % balance4Rw_all(i, run) = computeBalanceD(labelsalg4Rw, sensitive==1, k);
        ...
        balance6Rw_all(i, run) = computeBalanceD(labels6Rw, sensitive==1, k);

        tstart6Sym = tic;
        [labels6Sym, t8] = alg6Sym(W, D, F, k);
        time_alg6Sym(i, run) = toc(tstart6Sym);
        eigs_alg6Sym(i, run) = t8;
        balance6Sym_all(i, run) = computeBalanceD(labels6Sym, sensitive==1, k);
...
ylabel('Iterations');
legend('Algorithm 5', 'Algorithm 3', 'Location', 'northwest');
title('Convergence Iterations vs. Cluster Count');
grid on;